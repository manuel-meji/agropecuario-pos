...
export const generateEscPosReceipt = (sale: any, cart: any[], client?: any) => {
  ...
  cart.forEach(item => {
    ...
    let taxLetter = (item.product.taxRate === 0) ? 'E' : `G${item.product.taxRate ?? 13}`;
  ...
  const iva = (sale.totalTax || 0).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2});
